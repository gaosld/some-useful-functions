...
inline int GetBase64EncLen(int src_len);
inline int GetBase64DecLen(int src_len);
...
inline int GetBase64EncLen(int nLen)
{
	int nIndex = 0;
	int nBufferLen = 0;
	int nTempLen = 0;
	...
	// 计算长度
	for (nIndex = 0; nIndex < nTempLen - 3; nIndex += 3)
	{
		nBufferLen += 4;
	}
...
inline int GetBase64DecLen(int nLen)
{
	int nIndex = 0;
	int nBufferLen = 0;
	int nTempLen = 0;
	...
	// 计算长度
	for (nIndex = 0; nIndex < nTempLen - 4; nIndex += 4)
	{
		nBufferLen += 3;
	}
